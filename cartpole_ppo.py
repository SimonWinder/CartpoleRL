...
This implements the PPO algorithm with clipped surrogate objective and GAE.
"""
...
class PPOAgent:
    """
    PPO agent that learns to balance the CartPole using the clipped surrogate objective.
    """
...
def train(episodes=1000, max_timesteps=500, update_every=2048):
    """
    Train the PPO agent on CartPole.
    ...
    print(f"Training PPO agent for up to {episodes} episodes...")
...
def plot_training_progress(episode_rewards, window=50):
    ...
    plt.savefig('training_progress_ppo.png')
    print("Training progress plot saved as 'training_progress_ppo.png'")
    plt.show()
...
if __name__ == "__main__":
    ...
    torch.save(agent.policy.state_dict(), 'cartpole_ppo_policy.pth')
    print("\nTrained policy saved as 'cartpole_ppo_policy.pth'")
